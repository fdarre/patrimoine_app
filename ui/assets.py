...
def show_asset_management(
    ...
    with tab2:
        ...
        if not accounts:
            ...
            if not allocation:
                st.warning("Veuillez d'abord spécifier au moins une catégorie d'actif avec un pourcentage supérieur à 0%.")
            else:
                ...
                for i, (category, alloc_pct) in enumerate(allocation.items()):
                    with geo_tabs[i]:
                        st.info(f"Configuration de la répartition géographique pour la partie '{category}' ({alloc_pct}% de l'actif)")
                ...
                if is_composite and assets:
                    ...
                    st.warning("Le total des pourcentages des composants peut aller jusqu'à 100%. Le reste sera alloué directement selon l'allocation définie ci-dessus.")
                    ...
                    if components_total > 100:
                        ...
                        st.info(f"Total des composants: {components_total}%. Reste alloué directement: {100 - components_total}%")

                # Vérification globale
                all_geo_valid = all(sum(geo.values()) == 100 for cat, geo in geo_allocation.items() if cat in allocation)
